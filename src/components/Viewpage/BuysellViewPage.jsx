...
//               <img
//                 src={`${baseURL}/${bookDetail.bookImage}`}
...
//                   {bookDetail.delivery ? "Available" : "Not Available"}
//                 </p>
//                 <p className="mt-2 text-sm font-bold">Negotiable:</p>
//                 <p className="text-sm text-gray-700">
//                   {bookDetail.negotiable ? "Yes" : "No"}
//                 </p>
//               </div>
//               <div>
//                 {bookDetail?.postedBy?.name &&
//                 bookDetail?.postedBy?.phoneNumber ? (
//                   <>
//                     <h1>Posted By</h1>
//                     <h2>Name: {bookDetail.postedBy.name}</h2>
//                     <h2>Contact No.: {bookDetail.postedBy.phoneNumber}</h2>
//                   </>
//                 ) : (
//                   <p className="text-gray-500">
//                     Posted by information not available.
//                   </p>
//                 )}
//               </div>
//             </div>
...
const BuysellViewpage = () => {
  ...
  const handleBuySellBookForm = () => {
    navigate(`/buysell-form/${bookId}`); // Changed to use path parameter instead of query parameter
  };
  ...
  if (error) {
    return <div className="min-h-screen pt-24 flex justify-center items-center text-red-500">{error}</div>;
  }

  if (!bookDetail) {
    return <div className="min-h-screen pt-24 flex justify-center items-center">No book found</div>;
  }
      ...
      <div className="container mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-white shadow rounded p-4">
            ...
            <div className="flex justify-between items-center mt-4">
              <button className="p-2 bg-gray-300 rounded hover:bg-gray-400 transition">
                &lt;
              </button>
              <button className="p-2 bg-gray-300 rounded hover:bg-gray-400 transition">
                1
          ...
          <div className="bg-white shadow rounded p-6 md:col-span-2">
            ...
            <p className="text-gray-700 text-sm mt-2">
              {bookDetail.genre && Array.isArray(bookDetail.genre)
                ? bookDetail.genre.join(", ")
                : bookDetail.genre || "Genres not available"}
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              ...
              <div className="md:col-span-2">
                {bookDetail?.postedBy?.name && bookDetail?.postedBy?.phoneNumber ? (
                  <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                    <h3 className="font-bold text-md">Posted By</h3>
                    <p className="text-sm mt-1">Name: {bookDetail.postedBy.name}</p>
                    <p className="text-sm">Contact No.: {bookDetail.postedBy.phoneNumber}</p>
                  </div>
                ) : (
                  <p className="text-gray-500">
                    Posted by information not available.
